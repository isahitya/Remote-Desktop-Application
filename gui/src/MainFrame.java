...
{
private enum MODE{START_MODE,STOP_MODE};
...
int lm=0;
int tm=0;
//System.out.println((getWidth()-((lm+10)*2)-borderWidths)/2-5);
...
clientPanel.setBounds(lm+10+(getWidth()-((lm+10)*2)-borderWidths)/2+5,tm+10+32+10,(getWidth()-((lm+10)*2)-borderWidths)/2-5,285);
...
setLayout(null);
setSize(327,350);
/*moduleTitleLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
IPCaptionLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
portCaptionLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
IPTextField.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
portTextField.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
passwordCaptionLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
passwordTextField.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));*/
...
if(hostName.length()==0) errors.add("Hostname required");
if(port.length()==0) errors.add("Port required");
if(password.length()==0) errors.add("Password required");
if(errors.size()>0)
{
...
JOptionPane.showMessageDialog(MainFrame.this,sb.toString(),"Error",JOptionPane.ERROR_MESSAGE);
return;
}
else
{
...
//System.out.println("system chalu ho gaya");
int x;
java.util.Timer timer;
timer=new java.util.Timer();
timer.schedule(new Check(client,timer),0,50);
//System.out.println("Server se connection toot gaya");
}
...
timer.cancel();
timer.purge();
JOptionPane.showMessageDialog(MainFrame.this,"worng password","Error",JOptionPane.ERROR_MESSAGE);
}
if(x==2)
{
...
timer.cancel();
timer.purge();
JOptionPane.showMessageDialog(MainFrame.this,"Connection lost.","Error",JOptionPane.ERROR_MESSAGE);
}
if(x==3)
{
MainFrame.this.setVisible(true);
...
timer.cancel();
timer.purge();
JOptionPane.showMessageDialog(MainFrame.this,"Invalid IP address or port number.","Error",JOptionPane.ERROR_MESSAGE);
}
}
} 
}
...
message=new JLabel("<html>*Share the above-shown IP address, port number, and password with partner whom you want to allow to remotely control your computer.</html>");
try
{
InetAddress inetAddress = InetAddress.getLocalHost();
IPLabel=new JLabel(inetAddress.getHostAddress());
//IPLabel=new JLabel("111.111.111.111");
}catch(Exception e)
{
...
IPCaptionLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
portCaptionLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
IPLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
portLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
portLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
passwordCaptionLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));
passwordLabel.setBorder(BorderFactory.createLineBorder(new Color(255,0,0)));*/
...
MainFrame.this.setNotReady();//batman
...
outputStream.flush();
socket.close();
MainFrame.this.serverPanel.serverSocket=serverSocket;// bat
DisconnectFrame disconnectFrame=new DisconnectFrame();
new Server(serverSocket,port);
//System.out.println("ruk gaya Server stoped");
